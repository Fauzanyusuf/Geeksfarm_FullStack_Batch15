...
class SearchInput extends Component {
  ...
  handleSearch = async (e) => {
    ...
    const { setImages, setQuery } = this.props;
  ...
  render() {
    const { search } = this.state;
    return (
      <form onSubmit={this.handleSearch} className="flex mt-4">
        <input
...
export default class App extends Component {
  ...
  componentDidMount() {
    console.log("App mounted - ready to search images");
  }
  ...
  render() {
    ...
    return (
      <>
        <Jumbotron>
          <SearchInput setImages={this.setImages} setQuery={this.setQuery} />
        </Jumbotron>
